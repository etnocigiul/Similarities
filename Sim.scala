...
object Sim extends Serializable{
	...
	def main(args: Array[String]) {
		...
		outFile.coalesce(1).saveAsTextFile("output/sim_nID-" + nAut) 

		val elapsedTime = System.nanoTime() - startTime
		println("-----------------------------------\nTempo stimato di esecuzione: " + (elapsedTime / 1000000000.0) + " secondi, n° similarities: " + nSim + "\n-----------------------------------")
		}
	...
	*/
	def obtainSimilaritiesMatrix(authors: RDD[( (Long, Array[Long]), (Long, Array[Long]) )], default_preference: Double, default_jaccard: Double)  : RDD[(Long, Long, Double)]  = {
			...
			tmp1.union(tmp2).sortBy(x => (x._1, x._2), ascending = true)
	}
}
